...
(deftest touch-exec-test
  (let [check (fn [path filename]
                (as-> @state/app-state s
                  ...
                  (command/exec {:exec-fn :touch :state s :args (cons " " filename)})
                  (:state s)
...
(deftest touch-args-pred-test
  (let [check (fn [path filename]
                (as-> @state/app-state s
                  (assoc-in s [:nixy :filesystem :cwd] path)
                  (:valid (command/args {:args-fn :touch :state s :args filename}))))]
    (testing "touch"
      (testing "in /"
        (is (= nil (check [] " ")) "cannot create a file"))
      (testing "in /bin"
        (is (= nil (check ["bin"] " ")) "cannot create a file"))
      (testing "in /usr"
        (is (= true (check ["usr"] " ")) "can provide parameters")
        (is (= true (check ["usr"] " file")) "can provide a filename")
        (is (= true (check ["usr"] " file\n")) "can create a file")
        (is (= nil (check ["usr"] " existingfile\n")) "cannot overwrite another file")))))
